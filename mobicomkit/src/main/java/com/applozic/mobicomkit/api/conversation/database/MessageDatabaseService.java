...
public class MessageDatabaseService {
    ...
    public boolean deleteScheduledMessage(String messageKeyString) {
        SQLiteDatabase db = MobiComDatabaseHelper.getInstance(context).getWritableDatabase();
        ...
        if (duplicateCheck) {
            ...
            try {
                ...
                if (message.getGroupId() != null) {
                    queryClause = "channelKey ='" + message.getGroupId() + "'";
                } else {
    ...
    public int getUnreadMessageCountForChannel(Integer channelKey) {
        ...
        try {
            SQLiteDatabase db = dbHelper.getReadableDatabase();
            cursor = db.rawQuery("SELECT unreadCount FROM channel WHERE channelKey = " + "'" + channelKey + "'", null);
            cursor.moveToFirst();
            int unreadMessage = 0;
            if (cursor.getCount() > 0) {

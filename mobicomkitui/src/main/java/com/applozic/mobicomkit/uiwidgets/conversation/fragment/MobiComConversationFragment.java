...
abstract public class MobiComConversationFragment extends Fragment implements View.OnClickListener, GestureDetector.OnGestureListener, ContextMenuClickListener, ALRichMessageListener {
    ...
    protected ImageView sendButton, recordButton;
    protected ImageView attch_camera;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mainEditTextLinearLayout = list.findViewById(R.id.main_edit_text_linear_layout);
        audioRecordFrameLayout = list.findViewById(R.id.audio_record_frame_layout);
        ...
        recordButtonWeakReference = new WeakReference<ImageView>(recordButton);
        ...
        emoticonsBtn = list.findViewById(R.id.emoticons_btn);
        if (emojiIconHandler == null && emoticonsBtn != null) {
            emoticonsBtn.setVisibility(View.GONE);
        }
        replayRelativeLayout = list.findViewById(R.id.reply_message_layout);
        messageTextView = list.findViewById(R.id.messageTextView);
        galleryImageView = list.findViewById(R.id.imageViewForPhoto);
        nameTextView = list.findViewById(R.id.replyNameTextView);
        attachReplyCancelLayout = list.findViewById(R.id.imageCancel);
        messageDropDownActionButton = list.findViewById(R.id.message_drop_down);
        messageUnreadCountTextView = list.findViewById(R.id.message_unread_count_textView);
        imageViewRLayout = list.findViewById(R.id.imageViewRLayout);
        imageViewForAttachmentType = list.findViewById(R.id.imageViewForAttachmentType);
...
//        sendButton.setVisibility(alCustomizationSettings.isRecordButton() && (contact != null || channel != null) ? View.GONE : View.VISIBLE);
        ...
        attch_camera = individualMessageSendLayout.findViewById(R.id.attch_camera);
        ...
        sendButton.setOnClickListener(new View.OnClickListener() {
                                          @Override
                                          public void onClick(View view) {
//                                              Log.e("LLL_Message_Size1: ", messageList.size()+"  \nLast_Message_Size: "+messageList.get(messageList.size() - 1));
                                              if (alCustomizationSettings.isMessageFastScrollEnabled()) {
                                                  if (getActivity() == null) {
                                                      return;
                                                  }
//                                                  Log.e("LLL_Message_Size: ", messageList.size()+"  \nLast_Message_Size: "+messageList.get(messageList.size()));
//                                                  getActivity().runOnUiThread(new Runnable() {
//                                                      @Override
//                                                      public void run() {
//
                ...
                if (channel != null) {
                    ...
                    if (alCustomizationSettings.isGroupInfoScreenVisible() && !Channel.GroupType.GROUPOFTWO.getValue().equals(channel.getType()) && !Channel.GroupType.OPEN.getValue().equals(channel.getType())) {
                        ...
                        if (!TextUtils.isEmpty(userId)) {
                            ...
                            Log.e("LLLLL_User_ID: ", userId);
                            try {
                                Intent myIntent = new Intent(getContext(), Class.forName("com.TBI.Client.Bluff.Activity.Profile.OtherUserProfile"));
                                myIntent.putExtra("other_id", userId + "");
                                myIntent.putExtra("other_username", "");
...
//        recordButton.setVisibility(alCustomizationSettings.isRecordButton() && (contact != null || channel != null) ? isSendButtonVisible ? View.GONE : View.VISIBLE : View.GONE);
    ...
    protected void sendMessage() {
        if (channel != null) {
            Log.e("LLLL_channel: ", channel.getKey() + channel.getName());
            ...
            if (contact.isBlocked()) {
                Log.e("LLLL_block: ", String.valueOf(contact.isBlocked()));
                userBlockDialog(false, contact, false);
            } else {
                Log.e("LLLL_block: ", "Process_Message..");
                processSendMessage();
            }
        }
    }
    ...
    protected void processSendMessage() {
        if (!TextUtils.isEmpty(messageEditText.getText().toString().trim()) || !TextUtils.isEmpty(filePath)) {
            Log.e("LLLL_process_message: ", messageEditText.getText().toString().trim());
    ...
    public void updateMessageMetadata(final String keyString) {
        int i = -1;
        if (!messageList.isEmpty()) {
            for (Message message : messageList) {
        ...
        if (i != -1) {
            ...
            this.getActivity().runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Log.e("LLLLLL_Message_data: ", messageList.get(j).getMessage());
                    //Note: Removing and adding the same message again as the new sms object will contain the keyString.
                    if (recyclerDetailConversationAdapter != null) {
                        recyclerDetailConversationAdapter.notifyDataSetChanged();
                    }
        ...
        this.getActivity().runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                if (added) {
                    ...
                    messageList.add(message);
                    Log.e("LLLLLL_Update_Message: ", message.getMessage());
                    recyclerDetailConversationAdapter.notifyDataSetChanged();
            ...
            case 6:
                try {
                    ...
                    if (message.hasAttachment()) {
                        ...
                        imageViewForAttachmentType.setColorFilter(ContextCompat.getColor(ApplozicService.getContext(getContext()), R.color.apploizc_lite_gray_color));
                        messageImageLoader.setLoadingImage(R.drawable.applozic_map_offline_thumnail);
                        messageImageLoader.loadImage(LocationUtils.loadStaticMap(message.getMessage(), geoApiKey), galleryImageView);
                    } else {
                        Log.e("LLLLL_Message_reply: ", message.getMessage());
    ...
    public void updateUploadFailedStatus(final Message message) {
        Log.e("LLLLLL_Message_failed: ", message.getMessage());
        if (getActivity() == null) {
            return;
        }
        getActivity().runOnUiThread(new Runnable() {
                ...
                try {
                    ...
                    for (int index = 0; index < messageList.size(); index++) {
                        ...
                        if (view != null && !message.isCustom() && !message.isChannelCustomMessage()) {
                            ImageView createdAtTime = view.findViewById(R.id.msgstatus);
                            TextView status = view.findViewById(R.id.status);
                            createdAtTime.setImageDrawable(statusIcon);
                        }
                    }
                } catch (Exception ex) {
            ...
            public void run() {
                try {
                    ...
                    if (index != -1) {
                        ...
                        Log.e("LLLLLL_Message_data: ", message.getMessage());
                        recyclerDetailConversationAdapter.notifyDataSetChanged();
                    }
                } catch (Exception ex) {
                    Utils.printLog(getContext(), TAG, "Exception while updating delivery status in UI.");
    ...
    public void sendMessage(String message, Map<String, String> messageMetaData, FileMeta fileMetas, String fileMetaKeyStrings, short messageContentType, String filePath) {
        ...
        if (channel != null) {
            Log.e("LLLL_channel_: ", String.valueOf(channel));
        ...
        Log.e("LLLL_Message_: ", String.valueOf(messageToSend));
//        recyclerDetailConversationAdapter
                ...
                if (index != -1) {
                    ...
                    if (view != null) {
                        ...
                        if (createdAtTime != null && messageListItem.getKeyString() != null && messageListItem.isTypeOutbox() && !messageListItem.isCall() && !messageListItem.getDelivered() && !messageListItem.isCustom() && !messageListItem.isChannelCustomMessage() && messageListItem.getScheduledAt() == null
                            ...
                            createdAtTime.setImageDrawable(support.isSupportNumber(getCurrentUserId()) ? deliveredIcon : sentIcon);
                        }
                    }
                }
            }
...
// TODO :hard to read and suggested if we have very few event view
    ...
    public class DownloadConversation extends AsyncTask<Void, Integer, Long> {
        ...
        private void setWeakReferences() {
            ...
            sendButtonWeakReference = new WeakReference<ImageView>(sendButton);
        ...
        protected void onPreExecute() {
            ...
            if (!initial && messageList.isEmpty()) {
              ...
              /*  final AlertDialog alertDialog = new AlertDialog.Builder(getContext()).create();
                ...
                txtdelete = (Button) dialogLayout.findViewById(R.id.txtdelete);
                txtcancle = (Button) dialogLayout.findViewById(R.id.txtcancle);
                txttitle = (TextView) dialogLayout.findViewById(R.id.txttitle);
                txtmsg = (TextView) dialogLayout.findViewById(R.id.txtmsg);
                txtdelete.setText(getResources().getString(R.string.ok_alert));
                ...
                txtmsg.setText(R.string.sync_older_messages);
                alertDialog.setCancelable(true);
                alertDialog.create().show();*/
            }
        }
        ...
        protected void onPostExecute(Long result) {
            ...
            try {
                ...
                if (initial) {
                    ...
                    Log.d("mn13message", messageList.size() + "");
                    Log.d("mn13mes", messageList.size() + "");

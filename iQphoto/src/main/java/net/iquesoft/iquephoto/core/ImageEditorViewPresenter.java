...
//        mDrawingPaint.setStrokeJoin(Paint.Join.ROUND);
    ...
    void setupBitmapOnView(Bitmap bitmap, int width, int height) {
        ...
        mImageRect = new RectF(0, 0, mViewWidth, mViewHeight);
...
//        bitmap = Bitmap.createBitmap(bitmap, 0, 0, mViewWidth, mViewHeight);

        getViewState().setupImage(Bitmap.createScaledBitmap(bitmap, mViewWidth, mViewHeight, false), mImageMatrix, mImageRect);
    }
    ...
    void addText(Text text, FragmentActivity activity) {
        EditorText editorText = new EditorText(text, activity);
...
//                mRadialTiltShift.setHelpOvalEnabled(true);
    ...
    private Bitmap getBlurBitmap(Context context, Bitmap bitmap, int width, int height) {
        ...
        Bitmap scaledBitmap = Bitmap.createScaledBitmap(src, bitmap.getWidth(), bitmap.getHeight(), false);
